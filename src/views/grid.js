...
export default mnx.view.extend(
    _.extend(_.clone(mnx.utils.viewActions), _.clone(mnx.utils.viewLoading), _.clone(autoUtilEvents), {
        template: template,
        exportTemplate,
        regions: {
            filter: ".filter",
        ...
        initializePrepareOptions() {
            ...
            this.collection.filterOnServer = this.options.filterOnServer || false;
        ...
        initializeSearchListener() {
            ...
            this.listenTo(this.getRegion("filter").currentView, "search", () => {
                if (this.collection.filterOnServer) {
                    ...
                    this.getRegion("pagination").currentView && this.getRegion("pagination").currentView.gofirst(true);
        ...
        remove() {
            var selectedRow = this.getRegion("list").currentView.getSelectedRow("cid");
            if (!selectedRow) {
                return;
            }
        ...
        getDefaultActions() {
            ...
            if (this.options.filters.cols)
                a.push({
                    ico: "search",
                    title: "Pesquisar",
                    callback: (e) => $(".filter", this.$el).toggleClass("search-on"),
                });
