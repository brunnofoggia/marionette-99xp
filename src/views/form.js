...
export default sync.extend({
    ...
    if (this.model) {
      ...
      if (this.options.id) {
        ...
        if (this.model.morphState === this.initialRenderOnState) {
          ...
          this.listenToOnce(this.model, this.initialRenderOnState, () => {
            this.render();
          });
        }
      }
  ...
  showRequired($form) {
    ...
    // if(_.size(v) > 0) {
    var reqFields =
      _.size(v) > 0 ? _.map(_.keys(v), (i) => this.getFieldErrorName(i)) : [];
    $(
  ...
  isThereAnExistingError(fieldErrorName) {
    return (
      $('[class*="form-error-' + fieldErrorName + '"]', this.$el).length > 0
    );
  },
  clearError(field) {
    $(
  ...
  rmvPopupItem($el, collection) {
    ...
    Sk.popup.simple.confirm({
      ...
      msg:
        'Você confirma a exclusão do item "' + selectedModel.getTitle() + '" ?',
      confirm: "Confirmar",
      dataCancel: "Cancelar",
      callback: (status) => {
        if (status) {
